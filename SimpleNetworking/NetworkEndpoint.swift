...
extension NetworkEndpoint {
    ...
    public func call(_ service: NetworkService) -> FailablePromise<NetworkResponse> {
        switch service.task {
        case .get:
            return networking.perform(.get(from: fullServicePath(service)), with: service.headers)
        case let .post(data, contentType):
            return networking.perform(.post(data: data, type: contentType, to: fullServicePath(service)), with: service.headers)
        case let .download(destination):
            return networking.perform(.download(from: fullServicePath(service), savingTo: destination), with: service.headers)
        }
    }
}
